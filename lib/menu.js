            ...
            case 'Add Role':
                {
                    ...
                    await db.addRole(input.title, input.salary, input.department);
                    break;
                }
            case 'Add Employee':
                {
                    const input = await inquirer.prompt(addEmployee);
                    await db.addEmployee(input.first_name, input.last_name, input.role, input.manager);
                    break;
                }
            case 'Update Employee Role':
                {
                    const input = await inquirer.prompt(updateRole);
                    await db.updateRole(input.employee, input.role);
                    break;
                }
            case 'Update Employee Manager':
                {
                    const input = await inquirer.prompt(updateManager);
                    await db.updateManager(input.employee, input.manager);
                    break;
                }
            case 'Delete Department':
                {
                    const input = await inquirer.prompt(deleteDepartment);
                    await db.deleteDepartment(input.department);
                    break;
                }
            case 'Delete Role':
                {
                    const input = await inquirer.prompt(deleteRole);
                    await db.deleteRole(input.role);
                    break;
                }
            case 'Delete Employee':
                {
                    const input = await inquirer.prompt(deleteEmployee);
                    await db.deleteEmployee(input.employee);
                    break;
                }
            default:
                process.exit();
